...
- 半徑計算：半徑 r 必須是一個精確的數值（可以用根式表示，如 √5）
- 圓周點驗證：所有在 points_on_circle 中的點，其座標 (x, y) 必須精確滿足：
  √[(x - h)² + (y - k)²] = r
- 實際計算範例：
  * 如果圓心 O 在 (0, 0)，半徑 r = 5
  * 點 A 在 (5, 0)，距離 = √[(5-0)² + (0-0)²] = √25 = 5 ✓
  * 點 B 在 (3, 4)，距離 = √[(3-0)² + (4-0)²] = √25 = 5 ✓
  * 點 C 在 (0, 5)，距離 = √[(0-0)² + (5-0)²] = √25 = 5 ✓
  * 錯誤範例：點 D 在 (3, 3)，距離 = √18 ≈ 4.24 ✗ (不在圓上！)
- 使用極座標生成圓周點：如果圓心在 (h, k)，半徑為 r，角度為 θ，則圓周點座標為：
  x = h + r·cos(θ)
  y = k + r·sin(θ)
- 特別注意：生成題目時，先確定圓心座標和半徑，然後用極座標公式精確計算所有圓周點的座標
- 絕對禁止：不可以隨意放置點的座標後，再計算它到圓心的距離作為半徑，這會導致不一致！
